...
struct GS_EDGE_VORONOI_OUTPUT
{
	...
	float4 color	: COLOR;
	float4 dist		: TEXTURE0;
	float4 vec1		: TEXTURE1;
	float4 vec2		: TEXTURE2;
	float4 pos2		: TEXTURE3;
};
...
PS_VORONOI_OUTPUT VoronoiEdgePS(GS_EDGE_VORONOI_OUTPUT input)
{
	...
	float b = length(v1 - p);
	float c = length(v2 - p);
	float dist = sqrt(2*(pow(a,2)*pow(b,2)+pow(b,2)*pow(c,2)+pow(c,2)+pow(a,2))-(pow(a,4)+pow(b,4)+pow(c,4)))/(2*a);*/

	//float dist = abs(input.vec2.z - input.pos.z);
	
	//float dist = input.pos.z;

	//float dist = length(v1 - p);
	//float dist = length(v2 - p);
	...
	//output.color = float4(dist-1, 0.0f, 0.0f, 1.0f);

	output.depth = dist/3; // to avoid that depth gets greater than 1
...
PS_VORONOI_OUTPUT VoronoiVertexPS(GS_VERTEX_VORONOI_OUTPUT input)
{
	...
	output.color = input.color;
	output.dist = input.dist;
	output.depth = length(input.pos2.xyz - input.vertex.xyz);
	return output;
}
