...
struct GS_TRIANGLE_VORONOI_INPUT
{
	float4 pos		: POSITION;
	float3 normal	: NORMAL;
	float2 tex		: TEXCOORD;
...
GS_TRIANGLE_VORONOI_INPUT VoronoiTriangleVS(VS_VORONOI_INPUT input)
{
	GS_TRIANGLE_VORONOI_INPUT output;
	output.pos = mul(float4(input.pos, 1.0f), ModelViewProjectionMatrix);
...
[maxvertexcount(12)]
void VoronoiTriangleGS( triangle GS_TRIANGLE_VORONOI_INPUT input[3], inout TriangleStream<GS_TRIANGLE_VORONOI_OUTPUT> tStream)
{
	GS_VORONOI_INPUT triangle1[3];
	...
	else
	{
		...
		GS_VORONOI_INPUT interVec1 = triangle1[0];
		GS_VORONOI_INPUT interVec2 = triangle1[0];
