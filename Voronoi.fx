...
float3 interpolateFloat3(float4 p1, float4 p2, float3 f3_1, float3 f3_2, float sliceDepth)
{
	float zDist = p2.z - p1.z;
	float zWeight = (sliceDepth - p1.z)/zDist;
	return lerp(f3_1, f3_2, float3(zWeight, zWeight, zWeight));
}

float4 interpolateFloat4(float4 p1, float4 p2, float4 f4_1, float4 f4_2, float sliceDepth)
{
	float zDist = p2.z - p1.z;
	float zWeight = (sliceDepth - p1.z)/zDist;
	return lerp(f4_1, f4_2, float4(zWeight, zWeight, zWeight, zWeight));
}
...
void EdgeProjectOntoSlice(GS_EDGE_VORONOI_OUTPUT output, GS_EDGE_VORONOI_INPUT vec1, GS_EDGE_VORONOI_INPUT vec2, inout TriangleStream<GS_EDGE_VORONOI_OUTPUT> tStream, float fSliceDepth, bool bSliceDepthGreater)
{
	...
	if(a.x == 0 && a.y == 0)
	{
		float3 tex = interpolateFloat3(vec1.pos, vec2.pos, vec1.tex, vec2.tex, fSliceDepth);
		float4 color = interpolateFloat4(vec1.pos, vec2.pos, vec1.color, vec2.color, fSliceDepth);
		output.tex = tex;
		output.color = color;
...
void CalculateEdgeProjection(GS_EDGE_VORONOI_INPUT vec1, GS_EDGE_VORONOI_INPUT vec2, float fSliceDepth, inout TriangleStream<GS_EDGE_VORONOI_OUTPUT> tStream)
{
	...
	else// case when slice divides edge in 2 parts
	{
		...
		interVec.tex = interpolateFloat3(vec1.pos, vec2.pos, vec1.tex, vec2.tex, fSliceDepth);
		interVec.color = interpolateFloat4(vec1.pos, vec2.pos, vec1.color, vec2.color, fSliceDepth);
